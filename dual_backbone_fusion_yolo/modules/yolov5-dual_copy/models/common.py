...
class BiFPN_Add3(nn.Module):
    ...
    def forward(self, x):
        ...
        # weight = w / (torch.sum(w, dim=0) + self.epsilon)
        weight = [0.78, 0.22]
        return self.conv(weight[0] * x[0] + weight[1] * x[1])
    
class BiFPN_Add4(nn.Module):
    def __init__(self, c1, c2):
        super(BiFPN_Add4, self).__init__()
        # 设置可学习参数 nn.Parameter的作用是：将一个不可训练的类型Tensor转换成可以训练的类型parameter
        # 并且会向宿主模型注册该参数 成为其一部分 即model.parameters()会包含这个parameter
        # 从而在参数优化的时候可以自动一起优化
        # self.w = nn.Parameter(torch.ones(2, dtype=torch.float32), requires_grad=True)
...
class C3Bi(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
        self.Bi = BiLevelRoutingAttention(c2)
        
        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])

    def forward(self, x):
        return self.Bi(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1)))
...
class BiFormer(nn.Module):
    def __init__(self, in_chans=3, depth=[3, 4, 8, 3],  num_classes=1000, embed_dim=[64, 128, 320, 512],
...
class C3SPPF(nn.Module):
    ...
    def forward(self, x):
        return self.sppf(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1)))
...
class empty_(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5) -> None:
        super().__init__()
        self.n = n
...
class FeatureFusion(nn.Module):
    ...
    def forward(self, input_1, input_2):
        # Process input 1 through the first convolutional block
        out1 = nn.functional.relu(self.conv1(input_1))
        out1 = nn.functional.relu(self.conv2(out1))

        # Process input 2 through the second convolutional block
        out2 = nn.functional.relu(self.conv3(input_2))
        out2 = nn.functional.relu(self.conv4(out2))

        # Concatenate the outputs from both convolutional blocks
        out = torch.cat((out1, out2), dim=1)

        # Process the concatenated outputs through the third convolutional block
        out = nn.functional.relu(self.conv5(out))
        out = nn.functional.relu(self.conv6(out))

        # Upsample the output to match the input tensor sizes
