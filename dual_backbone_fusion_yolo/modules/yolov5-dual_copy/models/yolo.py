...
from models.common import empty_
    ...
    for i, (f, n, m, args) in enumerate(d['backbone1']+d['backbone2']+d['head']):  # 遍历backbone和head的每一层
        ...
        if i<backbone1depth:
            if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, conv_bn_relu_maxpool, Shuffle_Block,CARAFE,Concat3,CBAMC3, C3Bi,C3SPPF,empty_]:
                c1, c2 = ch[f], args[0]
                if c2 != no:  
        ...
        # depth gain 控制深度  如v5s: n*0.33   n: 当前模块的次数(间接控制深度)
            ...
            if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                     BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, conv_bn_relu_maxpool, Shuffle_Block,CBAM,CBAMC3, C3Bi,C3SPPF,empty_]:
                c12, c22 = ch2[f], args[0] #保存输出
                if c22 != no:  
                    c22 = make_divisible(c22 * gw, 8)  #保证通道是8的倍数
                args = [c12, c22, *args[1:]] 
                if m in [BottleneckCSP, C3, C3TR, C3Ghost,CBAM,CBAMC3, C3Bi]:
                ...
                for x in f:
                    if x==-1:
                        ...
                        c2p = ch[x]
                    c22 = c22+c2p
            elif m in [BiFPN_Add2, BiFPN_Add3, BiFPN_Add4]: #Concat：f是所有需要拼接层的索引，则输出通道c2是所有层的和
                c22 = 0
                for x in f:
                    if x==-1:
                        c2p = ch2[x]
