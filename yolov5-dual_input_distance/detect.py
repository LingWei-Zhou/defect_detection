...
from utils.torch_utils import select_device, time_sync, time_synchronized
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                if save_img or save_crop or view_img:  # Add bbox to image
                    ...
                    if (x <= 1242):
                        ...
                        map1x, map1y, map2x, map2y, Q = getRectifyTransform(375,1242, config)
                        ...
                        text_cxy = "*"
                        cv2.putText(im0, text_cxy, (int(x), int(y)), cv2.FONT_ITALIC, 0.6, (0, 0, 255), 1)
                        ...
                        text_x = "x:%.1fm" % (points_3d[int(y), int(x), 0]  * largue2)
                        text_y = "y:%.1fm" % (points_3d[int(y), int(x), 1] * largue2)
                        text_z = "z:%.1fm" % (points_3d[int(y), int(x), 2] * largue2)
                        text_dis = "dis:%.1fm" % (dis * largue1)
 

                        cv2.rectangle(im0, (int(xyxy[0] + (xyxy[2] - xyxy[0])), int(xyxy[1])),
                                    (int(xyxy[0] + (xyxy[2] - xyxy[0]) + 5 + 110), int(xyxy[1] + 90)),
                                    colors(int(cls)), -1)
                        cv2.putText(im0, text_x, (int(xyxy[0] + (xyxy[2] - xyxy[0]) + 5), int(xyxy[1] + 20)),
                                    cv2.FONT_ITALIC, 0.6, (255, 255, 255), 2)
                        cv2.putText(im0, text_y, (int(xyxy[0] + (xyxy[2] - xyxy[0]) + 5), int(xyxy[1] + 40)),
                                    cv2.FONT_ITALIC, 0.6, (255, 255, 255), 2)
                        cv2.putText(im0, text_z, (int(xyxy[0] + (xyxy[2] - xyxy[0]) + 5), int(xyxy[1] + 60)),
                                    cv2.FONT_ITALIC, 0.6, (255, 255, 255), 2)
                        cv2.putText(im0, text_dis, (int(xyxy[0] + (xyxy[2] - xyxy[0]) + 5), int(xyxy[1] + 80)),
                                    cv2.FONT_ITALIC, 0.6, (255, 255, 255), 2)
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / 'data/uav2850/test_images', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--source2', type=str, default=ROOT / 'data/uav2850/test_images2', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128.yaml', help='(optional) dataset.yaml path')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[1280], help='inference size h,w') #网络输入图片的大小
