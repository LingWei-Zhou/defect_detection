...
output_address='H:\刘对虹\data\image\training\image_my_2d\';


for i=213:213%代表要处理的图片的第一张和最后一张  4582
...
% figure(3);
% imshow(img_my_duibi,[])
% max_t=max(count_duibi)
% min_t=min(count_duibi);
% [col,row]=find(img_my_duibi>0);
% max_exp=255;min_exp=10;
% k=(max_exp-min_exp)/(min_t-max_t);b=max_exp-k*min_t;
% img_my_duibi(col,row)=k*img_my_duibi(col,row)+b;
% figure(4);
% imshow(img_my_duibi,[])
%% 方差图反转  相乘
...
max_exp=1;min_exp=0;
img_my1=img_my;
k=(max_exp-min_exp)/(min1-max1);b=max_exp-k*min1;
for mm=1:size(col,1)
...
% imwrite(img_write,Outputphoto_address);
fid = fopen(Outputphoto_address, 'wt');
for i = 1 : size(img_write, 1)
    for j = 1 : size(img_write, 2)
        fprintf(fid, '%g ', img_write(i, j));
    end
    fprintf(fid, '\n');
end
fid = fclose(fid);
